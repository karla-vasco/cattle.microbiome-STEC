...
## PERMDISP
wuni_disp <-betadisper(wuinfrac_dist, sample_data(physeq)$Phase, type=c("median"))
anova(wuni_disp)
...
supp.labs <- c("1B  R^2=0.14, p=0.001")
...
wu_1BL = ggscatter(beta_meta, x = "Axis.1", y = "Axis.2", color = "Phase", shape = "STEC_presence", palette = c("#E69F00","#D55E00","#56B4E9","#0072B2"), ellipse = TRUE, ellipse.level=.5,mean.point = F, mean.point.size = 5, star.plot = F) +
  labs(x = "PCoA 1 (16.7%)", y = "PCoA 2 (11.9%)", colour = "Phase", fill = "Phase", shape = "Phase") + 
  rremove("legend") + 
...
#2D beta
```{r}
physeq = subset_samples(physeq_long, Farm%in%c("2D"))
beta_wu <- ordinate(physeq, "PCoA", "wunifrac")
supp.labs <- c("2D R^2=0.13, p=0.001")
...
wu_2DL = ggscatter(beta_meta, x = "Axis.1", y = "Axis.2", color = "Phase", shape = "STEC_presence", palette = c("#E69F00","#D55E00","#56B4E9","#0072B2"), ellipse = TRUE, ellipse.level=.5,mean.point = F, mean.point.size = 5, star.plot = F) +
  labs(x = "PCoA 1 (10.4%)", y = "PCoA 2 (8.7%)", colour = "Phase", fill = "Phase", shape = "Phase") + 
  rremove("legend") + 
  facet_wrap(vars(Farm),nrow=1,ncol=1,scales="free",labeller = labeller(Farm = supp.labs))+
  theme_bw()+
...
beta_wu <- ordinate(physeq, "PCoA", "wunifrac")
supp.labs <- c("4D  R^2=0.06041, p=0.001")
...
wu_4DL = ggscatter(beta_meta, x = "Axis.1", y = "Axis.2", color = "Phase", shape = "STEC_presence", palette = c("#D55E00","#56B4E9","#0072B2"), ellipse = TRUE, ellipse.level=.5,mean.point = F, mean.point.size = 5, star.plot = F) +
  labs(x = "PCoA 1 (18%)", y = "PCoA 2 (10.4%)", colour = "Phase", fill = "Phase", shape = "Phase") + 
  rremove("legend") + 
  facet_wrap(vars(Farm),nrow=1,ncol=1,scales="free",labeller = labeller(Farm = supp.labs))+
  theme_bw()+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), legend.position = "none")+
...
physeq = subset_samples(physeq_long, Farm%in%c("8B"))
beta_wu <- ordinate(physeq, "PCoA", "wunifrac")
supp.labs <- c("8B  R^2=0.21, p=0.001")
...
wu_8B = ggscatter(beta_meta, x = "Axis.1", y = "Axis.2", color = "Phase", fill = "Phase", shape = "STEC_presence", palette = c("#E69F00","#D55E00","#56B4E9","#0072B2"), ellipse = TRUE, ellipse.level=.5,mean.point = F , mean.point.size = 5, star.plot = F) +
  labs(x = "PCoA 1 (29.6%)", y = "PCoA 2 (12.9%)", colour = "Phase", fill = "Phase", shape = "Phase") + 
 # rremove("legend") + 
  facet_wrap(vars(Farm),nrow=1,ncol=1,scales="free",labeller = labeller(Farm = supp.labs))+
...
#9D beta
```{r}
physeq = subset_samples(physeq_long, Farm%in%c("9D"))
beta_wu <- ordinate(physeq, "PCoA", "wunifrac")
supp.labs <- c("9D  R^2=0.13, p=0.003")
...
wu_9Dt = ggscatter(beta_meta, x = "Axis.1", y = "Axis.2", color = "Phase", shape = "STEC_presence", fill = "Phase",palette = c("#E69F00","#D55E00","#56B4E9","#0072B2"), ellipse = TRUE, ellipse.level=.5,mean.point = F, mean.point.size = 5, star.plot = F) +
  labs(x = "PCoA 1 (17.9%)", y = "PCoA 2 (12.7%)", colour = "Phase", fill = "Phase", shape = "Phase") + 
 # rremove("legend") + 
  facet_wrap(vars(Farm),nrow=1,ncol=1,scales="free",labeller = labeller(Farm = supp.labs))+
  theme_bw()+
...
#All farms beta diversity
```{r}
beta_scatter = as.data.frame(beta_wu[["vectors"]])
beta_meta = merge(beta_scatter,metadata,by = 0,all=F)
beta_meta$Phase = as.factor(beta_meta$Phase)
pmain_wu = ggscatter(beta_meta, x = "Axis.1", y = "Axis.2", color = "Phase", palette = c("#E69F00","#D55E00","#56B4E9","#0072B2"), ellipse = TRUE, ellipse.level=.5,mean.point = F, mean.point.size = 5, star.plot = F) +
  labs(x = "Principal Coordinate 1 (16.5%)", y = "Principal Coordinate 2 (8.6%)", colour = "Phase", fill = "Phase", shape = "Phase") + 
  rremove("legend") + 
  facet_wrap(vars(Farm),nrow=1,ncol=5,scales="free")+
  theme_bw()+
...
#legend alpha OLD
```{r}
plot_leg <-  df_alpha %>% #shannon_box, chao1_box
  mutate(x_axis = fct_relevel(Phase, "1","2","3","4")) %>% #Optional if you need a specific x_axis order
  ggplot(aes(x = Phase, y = Shannon, fill = Farm)) +
  geom_boxplot(outlier.shape = NA, color = "black") +
  facet_wrap(vars(Farm),nrow=1,ncol=5,scales="free", labeller = labeller(Farm = supp.labs))+
  geom_point(alpha=0.2, position = position_jitterdodge()) +
  labs(y="Shannon", x="Phase", shape = "STEC shedding") + 
    theme_bw() + 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + 
  theme(legend.position = "top",axis.title.x=element_blank()) + 
 # stat_compare_means(comparisons = my_comparisons, step.increase = .15, symnum.args = list(cutpoints = c(0, 0.0001, 0.001, 0.01, 0.05, 1), symbols = c("****", "***", "**", "*", "ns")), tip.length=.02, hide.ns = FALSE) + 
...
# Convert to a ggplot and print
legend_farm = as_ggplot(leg)
legend_farm

plot_leg_shape <-  df_alpha %>% #shannon_box, chao1_box
  mutate(x_axis = fct_relevel(Phase, "1","2","3","4")) %>% #Optional if you need a specific x_axis order
  ggplot(aes(x = Phase, y = Shannon, shape=STEC.shedding, color=STEC.shedding)) +
 # geom_boxplot(outlier.shape = NA, color = "black") +
  facet_wrap(vars(Farm),nrow=1,ncol=5,scales="free", labeller = labeller(Farm = supp.labs))+
  geom_point(alpha=1, position = position_jitterdodge()) +
  labs(y="Shannon", x="Phase", shape = "STEC shedding",color="STEC shedding") + 
    theme_bw() + 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + 
  theme(legend.position = "top",axis.title.x=element_blank()) + 
 # stat_compare_means(comparisons = my_comparisons, step.increase = .15, symnum.args = list(cutpoints = c(0, 0.0001, 0.001, 0.01, 0.05, 1), symbols = c("****", "***", "**", "*", "ns")), tip.length=.02, hide.ns = FALSE) + 
...
# Extract the legend. Returns a gtable
leg_shape <- get_legend(plot_leg_shape)

# Convert to a ggplot and print
legend_shape = as_ggplot(leg_shape)
legend_shape
```

#legend beta
```{r}
plot_leg_WU = ggscatter(beta_meta, x = "Axis.1", y = "Axis.2", color = "Phase", fill = "Phase", shape = "STEC.shedding", palette = c("#E69F00","#D55E00","#56B4E9","#0072B2"), ellipse = TRUE, ellipse.level=.5,mean.point = F , mean.point.size = 5, star.plot = F) +
  labs(x = "PCoA 1 (29.6%)", y = "PCoA 2 (12.9%)", colour = "Phase", fill = "Phase", shape = "STEC shedding") + 
 # rremove("legend") + 
  facet_wrap(vars(Farm),nrow=1,ncol=1,scales="free",labeller = labeller(Farm = supp.labs))+
  theme_bw()+   theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), legend.position = "top") + scale_shape_manual (values = c(24,25,23,20,3))
...
#Diversity plot OLD
```{r}
legend_alpha_all = ggarrange(legend_farm,legend_shape, nrow = 1,ncol=2,align="hv")
beta_div_long = ggarrange(wu_1BL,wu_2DL,wu_4DL,wu_8B,wu_9Dt, nrow = 1,ncol=5,align="hv")
long_div = ggarrange(legend_alpha_all, shannon_box, chao1_box, legend_wuL, beta_div_long, nrow = 5, ncol = 1, align="hv", heights = c(.3,1,1,.3, 1.5), labels = c("","A","B","","C"))
long_div
```
...
pdf(file='Longitudinal_diversity6_presence.pdf',width=setWidth,height=7,pointsize=setFontSize)
long_div
dev.off()
